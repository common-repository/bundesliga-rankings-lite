<?php
/*
Plugin Name: Bundesliga Football Rankings Lite
...
function BUNDESLIGASPRLPageURL() {
 $BUNDESLIGASPRLpageURL = 'http';
 $BUNDESLIGASPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $BUNDESLIGASPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $BUNDESLIGASPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $BUNDESLIGASPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_bundesligasprlstats() 
{
?>
<h2>Bundesliga Rankings Lite</h2>
<?php bundesligasprl_stats(); ?>
<?php
}

function bundesligasprlstats_install()
{
register_sidebar_widget(__('Bundesliga Rankings Lite'), 'widget_bundesligasprlstats'); 
}
add_action("plugins_loaded", "bundesligasprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'bundesligasprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'bundesligasprl_stats_remove' );

function bundesligasprl_stats_install() 
{
...
add_option("bundesligasprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("bundesligasprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("bundesligasprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE BUNDESLIGALEAGUE Power Rankings Installation", "Hi\n\nLITE BUNDESLIGALEAGUE Power Rankings Activated at \n\n".BUNDESLIGASPRLPageURL()."\n\nBUNDESLIGALEAGUE Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function bundesligasprl_stats_remove() 
{
/* Deletes the database field */
delete_option('bundesligasprl_scroll_text_color');
delete_option('bundesligasprl_scroll_text_color1');
delete_option('bundesligasprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'bundesligasprl_stats_admin_menu');

function bundesligasprl_stats_admin_menu() {

add_options_page('Bundesliga Rankings Lite', 'Bundesliga Rankings Lite Settings', 'administrator', 'bundesliga-power-rankings-lite.php', 'bundesligasprl_stats_plugin_page');
}
}
function bundesligasprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('bundesligasprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('bundesligasprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('bundesligasprl_scroll_text_color2'));

$myads = "http://www.ibet.ws/bundes_stats_rankings_magpie_lite/bundesliga_stats_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $myads;
$rss = bundesligasprl_fetch_rss( $url );
...
function bundesligasprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>Bundesliga Rankings Lite Options Page</h2>
...
<p>
<?php echo bundesligasprl_stats(); ?>
</p>
<?php echo $myads; ?>
<!-- Start Advanced Plugins List -->
  <h2>If You Want MORE Stats and Information:</h2>
  <p>A93D Offers FREE upgrades for this stats package, that allow you to display advanced and more complete BUNDESLIGALEAGUE Power Rankings.
  ...
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/bundesliga-rankings.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/bundesliga-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
...
<p><strong>Soccer / Football / Futbol</strong><br />
  <strong>Premier League rankings</strong> <a href="http://wplatest5.thoseamazingparks.com">Preview</a> | <a href="http://www.ibet.ws/download/premier-league-rankings.zip">Download Now</a> - Complete Premier League Rankings for all 20 Teams.</p>
